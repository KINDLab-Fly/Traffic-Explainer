...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask, output_attentions=False):
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, seq_mask, output_attentions=False):
        if output_attentions:
            norm_x = self.residual_connections[0].norm(x)
            att_out, att_scores = self.self_attention_block(norm_x, norm_x, norm_x, seq_mask, output_attentions=True)
            x = x + self.residual_connections[0].dropout(att_out)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask, output_attentions=False):
        ...
        for layer in self.layers:
            if output_attentions:
                x, attention = layer(x, mask, output_attentions=True)
...
class Transformer(nn.Module):
    ...
    def encode(self, seq, mask, output_attentions=False):
        # (batch, seq_len, d_model)
        seq = self.seq_pos(seq)

        return self.encoder(seq, mask, output_attentions=output_attentions)
...
def build_transformer(seq_vocab_size: int, seq_seq_len: int, seq_pack_len: int, class_size: int, d_model: int=64, N: int=3, h: int=4, dropout: float=0.1, d_ff: int=128, header = False) -> Transformer:
    ...
    for _ in range(N):
        ...
        # encoder_self_attention_block = MLPBlock(d_model, h, dropout)
...
class explainer_model(nn.Module):
    def __init__(self, config):
        ...
        self.mask_header = torch.nn.Parameter(torch.ones(config.seq_vocab_size))
        self.mask_payload = torch.nn.Parameter(torch.ones(config.seq_vocab_size))
